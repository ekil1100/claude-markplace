...
4. 检查 diff 中是否混入“未修改代码行仅因格式化导致的改动”；若存在，按 review.md 对“代码规范”/“提交原子性”降档
5. 严格按 review.md 的 5 项逐项打分（只能选固定档位，就低不就高）
6. 对照 mistakes.md 检查是否命中已知错误模式
7. 返回：评分表 + 结论（通过 ✅ ≥95 / 不通过 ❌ <95）+ 具体修复建议
