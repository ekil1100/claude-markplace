...
Worker 每次提交前，必须由 Reviewer 审查代码。总分 100 分，**90 分以上才能提交**，否则打回修改。
...
| 分数 | 标准 |
|------|------|
| 15 | 充分理解 V8 实现，功能对等，适配合理 |
| 12 | 理解 V8 实现，小部分功能缺失或适配粗糙 |
| 9 | 部分参考了 V8，但有明显误解或遗漏 |
| 6 | 看起来没有认真读 V8 源码 |
| 0 | 完全凭空实现，与 V8 设计不符 |
...
Review 结果的处理流程（≥90 通过、<90 打回、3 轮未通过 escalation）由 Planner 控制。
